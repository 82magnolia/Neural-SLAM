...
def _noisy_action_impl(
    ...
    # For each step, a drift can randomly happen
    if action == 0:  ## Forward
        prob = random.random()
        if prob < 1 / 3:  # No drift
            dx, dy, do = actuation_noise_fwd.sample()[0][0]
        elif 1 / 3 < prob < 2 / 3:  # Drift left
            rot_dx, rot_dy, rot_do = actuation_noise_left.sample()[0][0]
            dx, dy, do = actuation_noise_fwd.sample()[0][0]
            dx, dy, do = dx + rot_dx, dy + rot_dy, do + rot_do
        else:  # Drift right
            rot_dx, rot_dy, rot_do = actuation_noise_right.sample()[0][0]
            dx, dy, do = actuation_noise_fwd.sample()[0][0]
            dx, dy, do = dx + rot_dx, dy + rot_dy, do + rot_do
